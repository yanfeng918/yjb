package com.yongjinbao.houseProperty.controller;
...
import com.yongjinbao.houseProperty.dto.StateStatusDto;
...
import com.yongjinbao.houseProperty.vo.BrowseHouseInfoVO;
...
import com.yongjinbao.houseProperty.vo.StateStatusVO;
...
public class HouseInfoPropertyController {
    ...
    @Inject
    private IHouseInfoPropertyService houseInfoService;
    ...
    @ResponseBody
    public Pager<HouseInfoPropertyAndFavouriteStatusVO> getHouseInfoList(@ModelAttribute("getHouseInfoDto") GetHouseInfoDto getHouseInfoDto, HttpServletRequest request){
...
//        getHouseInfoDto.setMember_id(memberService.getMemberId(request));
        Pager<HouseInfoPropertyAndFavouriteStatusVO> pager=houseInfoService.getHouseInfo(getHouseInfoDto);
        return pager;
     }
    /**
     * 条件查询房屋信息列表
    ...
    @ResponseBody
    public Pager<HouseInfoProperty> getReleaseHouseInfo(@ModelAttribute("getHouseInfoDto") GetHouseInfoDto getHouseInfoDto, HttpServletRequest request){
        getHouseInfoDto.setMember_id(houseInfoService.getMemberId(request));
        Pager<HouseInfoProperty> pager=houseInfoService.getReleaseHouseInfo(getHouseInfoDto);
        return pager;
    }
	...
	public @ResponseBody
	HouseInfoProperty browseHouseInfo(long houseInfo_id, HttpServletRequest request) {
		BrowseHouseInfoVO vo = houseInfoService.getBrowseHouseInfoVO(houseInfo_id, request);
		if(vo.isBought()||!vo.isBalanceEnough()){
			return vo.getHouseInfoProperty();
		}
		//获取房源信息
		HouseInfoProperty houseInfoProperty = new HouseInfoProperty();
		houseInfoProperty = houseInfoService.browseHouseInfoByHouseId(houseInfo_id);
		Member loginMember = memberService.getCurrent(request);
		//自定义房源，如果查看3次后自动将所有收转入系统账号中【该逻辑包含在以下】
		Member houseInfoMember = houseInfoService.getHouseInfoMember(houseInfoProperty, false);
		...
		if(houseInfoMember.getId()==systemMember.getId()){
			houseInfoService.updateBalanceInfo(houseInfoMember, 
					Float.parseFloat(String.valueOf(houseInfoProperty.getInfoPrice())), UpdateBalanceDto.INCREASE);
		}else{
			houseInfoService.updateBalanceInfo(houseInfoMember, 
					Float.parseFloat(String.valueOf(houseInfoProperty.getInfoPrice()*0.7)), UpdateBalanceDto.INCREASE);
			houseInfoService.updateBalanceInfo(systemMember, 
					Float.parseFloat(String.valueOf(houseInfoProperty.getInfoPrice()*0.3)), UpdateBalanceDto.INCREASE);
		}
		...
		houseInfoService.updateBalanceInfo(loginMember, 
				Float.parseFloat(String.valueOf(houseInfoProperty.getInfoPrice())), UpdateBalanceDto.REDUCE);
		//更改收支明细、并写入我的查看房源信息
		houseInfoService.addIncomeExpenseAndBrowseInfo(houseInfoMember, loginMember, houseInfoProperty);
