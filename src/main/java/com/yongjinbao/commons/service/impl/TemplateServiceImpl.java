...
import com.yongjinbao.beans.Template;
import com.yongjinbao.beans.Template.Type;
import com.yongjinbao.commons.service.ITemplateService;
import com.yongjinbao.utils.CommonAttributes;

/**
 * Service - 模板
 */
@Service("templateServiceImpl")
public class TemplateServiceImpl implements ITemplateService, ServletContextAware {
	...
	public List<Template> getAll() {
		try {
			File shopxxXmlFile = new ClassPathResource(CommonAttributes.YJB_XML_PATH).getFile();
			Document document = new SAXReader().read(shopxxXmlFile);
			List<Template> templates = new ArrayList<Template>();
			List<Element> elements = document.selectNodes("/yjb/template");
	...
	public List<Template> getList(Type type) {
		if (type != null) {
			try {
				...
				Document document = new SAXReader().read(shopxxXmlFile);
				List<Template> templates = new ArrayList<Template>();
				List<Element> elements = document.selectNodes("/yjb/template[@type='" + type + "']");
				for (Element element : elements) {
					Template template = getTemplate(element);
					templates.add(template);
				}
	...
	public Template get(String id) {
		try {
			File shopxxXmlFile = new ClassPathResource(CommonAttributes.YJB_XML_PATH).getFile();
			Document document = new SAXReader().read(shopxxXmlFile);
			Element element = (Element) document.selectSingleNode("/yjb/template[@id='" + id + "']");
			Template template = getTemplate(element);
			return template;
		} catch (Exception e) {
			e.printStackTrace();
